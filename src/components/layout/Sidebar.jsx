...
import {
  ...
  FileSpreadsheet, BarChart3, Settings, ChevronsLeft, ChevronsRight, X, LogOut, BookOpen
} from 'lucide-react'
...
export default function Sidebar({ mobileOpen, onCloseMobile }) {
  ...
  const adminRole = currentUser?.role === 'admin' ? 'Administrator' : 'Admin'
          ...
          <div className={cn("flex items-center gap-3 mb-2", collapsed && "justify-center")}>
            ...
            {!collapsed && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[var(--color-text-muted)] truncate">{adminRole}</p>
              </div>
            )}
          </div>
          <button
